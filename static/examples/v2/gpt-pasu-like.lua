...
function spawnPASUTarget()
    spawnTarget({
        size = { radius = 0.12, height = 0.0 },
        position = { x = randomFloat(-3, 3), y = randomFloat(-2, 2), z = randomFloat(-2, 2) },
        hp = 1, -- Single hit to destroy
...
function createPASUMovement()
    ...
    local frequency = randomFloat(2, 5)              -- Movement frequency
    ...
    return function(self, elapsedTime, delta)
        ...
        local x = (amplitude * direction * math.cos(frequency * elapsedTime + randomOffset)) +
            ((amplitudeY * 2) * math.cos(frequencyY * 3 * elapsedTime))
        local y = amplitudeY * math.cos(frequencyY * elapsedTime + randomOffset)
        local z = (amplitude * direction * math.sin(frequency * elapsedTime + randomOffset)) +
